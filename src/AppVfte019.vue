...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte019" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" />
...
export default {
  ...
  methods: {
    ...
    loadDataCategories(loading,callback) {
      ...
      if(!loading) return;
      let jsondata = {names: ["tkbranchtype"]};
    ...
    resetDataCategories(lang) {
      ...
      let tkbranchtype;
      let tk_category = this.dataChunk["tkbranchtype"];
      if(tk_category) {
        tkbranchtype = tk_category.map((item) => { return { id: item.typeid, text: "TH"==lang?item.nameth:item.nameen } });
      }
      if(tkbranchtype) this.dataCategory.tkbranchtype = tkbranchtype;
    },
    dataSelected(item,action) {
      //listen action from search form
      console.log("App: dataSelected",item,"action",action);
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({site: item.site, branch: item.branch});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({branchdesc: item.branch+"-"+item.nameen, site: item.site, branch: item.branch});
      }
    },
    dataInsert(filters) {
      //listen action from search form
